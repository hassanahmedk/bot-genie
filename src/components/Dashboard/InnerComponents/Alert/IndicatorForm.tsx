...
const IndicatorForm: any = ({ indicators, extraClass, setIndicators }: { indicators:any, setIndicators:any,  extraClass:any }) => {
  ...
  const handleEditIndicator = (index: number) => {
    setIndicatorsList((prevList) => prevList.map((indicator, i) => {
      if(i === index){
        ...
        <div key={index} className="border border-gray-400 p-4 rounded flex flex-col gap-2">
          <div className="flex justify-between">
           ...
           <div className="flex gap-2">
            {
              !indicator.editState && 
              <p onClick={() => enableEditIndicator(index)} className="cursor-pointer text-green-800">Edit</p>
            }
          ...
          <div className="flex gap-2">
            <FormControl fullWidth>
              <TextField
                ...
                value={indicator.editState ? editData.interval : (indicator.inputParams.interval || '')}
                onChange={indicator.editState ? ()=>handleEditDataChange(event, "interval") : undefined}
            ...
            <FormControl fullWidth>
              <TextField
                ...
                variant="outlined"
                // if the indicator is in edit state then get value from edit data else from the indicator 
                value={indicator.editState ? editData.backtrack : (indicator.inputParams.backtrack || '')}
                onChange={indicator.editState ? ()=>handleEditDataChange(event, "backtrack") : undefined}
                disabled={!indicator.editState}
              />
            </FormControl>
              ...
              key !== 'interval' && key !== 'backtrack' ? (
                <FormControl fullWidth key={key}>
                  <TextField
                    ...
                    onChange={indicator.editState ? ()=>handleEditDataChange(event, key) : undefined}
                    disabled={!indicator.editState}
                  />
                </FormControl>
              ) : null
          ...
          </div>
          {
            ...
            <div className="flex gap-4 items-center self-end">
              <p className="cursor-pointer text-primary-800" onClick={() => disableEditIndicator(index)}>Cancel</p>
              <Button title="Update" size="sm" type="secondary" onClick={() => handleEditIndicator(index)}/>
            </div>
          }
        </div>
      ))}
